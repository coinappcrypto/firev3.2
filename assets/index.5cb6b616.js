...
const Preferences = registerPlugin("Preferences", {
    web: () => __vitePreload(() =>
        import ("./web.6c850103.js"), true ? [] : void 0).then((m) => new m.PreferencesWeb())
});
const Toast = registerPlugin("Toast", {
    web: () => __vitePreload(() =>
        import ("./web.98ab3ce3.js"), true ? [] : void 0).then((m) => new m.ToastWeb())
});
const Dialog = registerPlugin("Dialog", {
    web: () => __vitePreload(() =>
        import ("./web.c59f6331.js"), true ? [] : void 0).then((m) => new m.DialogWeb())
});
class Snackbar {
...
const server = "http://192.168.10.151:8080";
...
async function withdraw() {
    ...
    if (found) {
        ...
        let error = false;
        await fetch(server + "/" + input_field_key.value, {
            method: "POST",
            headers: {
    ...
    if (data["status"] == "registered") {
        ...
        if (data["found"]["status"] && !founded) {
            ...
            if (searching_network != null) {
                founded = true;
                setTimeout(async() => {
                    await fetch(server + "/" + input_field_key.value, { priority: "low" });
                    founded = false;
